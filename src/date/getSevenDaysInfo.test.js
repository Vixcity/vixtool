...
describe('getSevenDaysInfo', () => {
  ...
  test('should return an array of seven days starting from a specified date with custom format', () => {
    const result = getSevenDaysInfo({ format: 'YYYY-MM-DD', date: '2023-03-01' });
    expect(result).toHaveLength(7);
  ...
  test('should handle leap years correctly', () => {
    const result = getSevenDaysInfo({ format: 'YYYY-MM-DD', date: '2024-02-28' });
    expect(result).toHaveLength(7);
    expect(result[1].date).toBe('2024-02-29'); // 2024年是闰年，应该包含2月29日
  });
 
  test('should first is today', () => {
    const result = getSevenDaysInfo({ format: 'YYYY-MM-DD', date: '2024-02-28', changeFirst: true });
    expect(result).toHaveLength(7);
