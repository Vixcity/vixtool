...
describe("groupByAndNest", () => {
  // 测试：非数组输入应抛出错误
  it("should throw an error when the array parameter is not an array", () => {
    const invalidInput = {};
    expect(() => {
  ...
  it("should return the original array when groupByKeys is undefined or empty", () => {
  ...
  // 测试：根据单个属性分组和嵌套
  it("should group and nest objects by a single property", () => {
    const array = [
  ...
  // 测试：根据多个属性分组和嵌套
  it("should group and nest objects by multiple properties", () => {
    const array = [
  ...
  // 测试：数组对象已包含嵌套对象的情况
  it("should handle an array with nested objects", () => {
    const array = [
  ...
  // 测试：使用自定义的嵌套键
  it("should use a custom childrenKey for nesting", () => {
    const array = [
      { type: "fruit", name: "apple" },
      { type: "fruit", name: "banana" },
    ];
  ...
  // 测试：忽略对象缺少分组键属性的情况
  it("should ignore objects missing the groupByKeys properties", () => {
    const array = [
      { type: "fruit", name: "apple" },
  ...
  // 测试：分组键为空字符串的情况
  it("should handle cases where the groupByKeys is an empty string", () => {
    const array = [
      { "": "fruit", name: "apple" },
      { "": "fruit", name: "banana" },
    ];
  ...
  // 测试：分组键为非字符串的情况
  it("should handle non-string groupByKeys", () => {
    const array = [
      { type: 1, name: "apple" },
      { type: 1, name: "banana" },
    ];
  ...
  // 测试：删除分组键后的对象处理
  it("should remove the groupByKeys from nested objects", () => {
    const array = [
      { type: "fruit", name: "apple" },
      { type: "fruit", name: "banana" },
    ];
  ...
  // 测试：多个分组键的顺序
  it("should not be affected by the order of groupByKeys", () => {
    const array = [
      { type: "fruit", color: "red", name: "apple" },
  ...
  it("should use 'children' as the default childrenKey", () => {
    const array = [
      { type: "fruit", name: "apple" },
      { type: "fruit", name: "banana" },
    ];
  ...
  // 测试：数组元素不是对象时的情况
  it("should throw an error when array elements are not objects", () => {
